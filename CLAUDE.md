...
| 그림 | 파일 | 쓰는 곳 |
|---|---|---|
| 손으로 찍은 도트 곰 | `public/favicon.svg` | **16~32px** — 파비콘 전용 |
| 도트화한 일러스트 | `public/avatar.webp` | **56px 이상** — 홈 히어로, 소개, OG |

크기마다 살아남는 그림이 다르다. 일러스트는 파비콘 크기에서 뭉개지고,
16x16 도트 곰은 큰 자리에서 밋밋하다. **섞어 쓰지 않는다.**
둘 다 도트라 언어는 같다.

아바타는 **64px 격자 / 24색**으로 도트화한다(`scripts/gen-brand-assets.mjs`).
48px 이하로 내리면 안경과 표정이 뭉개져 "누구인지"가 사라진다.
색 수를 안 줄이면 도트가 아니라 그냥 흐릿한 저해상도 사진이 된다.

원형 마스크는 **64px 격자에서** 적용해 테두리도 픽셀 계단으로 만든다.
그래서 CSS `border-radius` 를 쓰지 않는다 — 매끈한 곡선이 안쪽 픽셀과 어긋난다.
확대는 `image-rendering: pixelated`.

전부 `npm run brand` 로 `assets/avatar-source.webp` 에서 재생성된다.

...
| 그림 | 파일 | 쓰는 곳 |
|---|---|---|
| 도트 곰 | `public/favicon.svg` | **16~32px** — 파비콘 전용 |
| 일러스트 | `public/avatar.webp` | **56px 이상** — 홈 히어로, 소개, OG(`og-default.png`) |

크기마다 살아남는 그림이 다르다. 일러스트는 파비콘 크기에서 뭉개지고,
도트 곰은 큰 자리에서 밋밋하다. **섞어 쓰지 않는다.**

아바타는 `border-radius: 50%` 로, **이 사이트에서 유일하게 둥근 요소다**(`--radius: 0` 의 예외).
원본이 원형 구성이고, 사각으로 자르면 흰 모서리가 다크모드에서 밝은 사각형으로 튄다.
